...
<pre>
  <% semantic_form_for @post do |form| %>
    ...
    <% form.buttons do %>
      <%= form.commit :button_html => { :class => "primary" } %>
    <% end %>
  <% end %>
</pre>
...
<pre>
  Formtastic::SemanticFormBuilder::Base.i18n_lookups_by_default = true
</pre>
...
<pre>
  Formtastic::SemanticFormBuilder::Base.i18n_lookups_by_default = false
</pre>
...
<pre>
  <% semantic_form_for @post do |form| %>
    ...
    <% end %>
    <% form.buttons do %>
      <%= form.commit true %>                # => "Save changes" (if we are in edit that is...)
    <% end %>
  <% end %>
</pre>
...
@Formtastic::SemanticFormHelper::Base.builder = MyCustomBuilder@
